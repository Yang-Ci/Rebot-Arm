...
class JointStatePublisher:
    ...
    def publish(self) -> None:
        ...
        if self._hardware.has_gripper:
            g_pos, g_vel, _g_torque, _g_status = self._hardware.get_gripper_state()
            msg.name = list(msg.name) + ["finger_left"]
            msg.position = list(msg.position) + [float(g_pos) * _GRIPPER_HALF_VISUAL_SCALE]
            msg.velocity = list(msg.velocity) + [float(g_vel) * _GRIPPER_HALF_VISUAL_SCALE]
            msg.effort = list(msg.effort) + [0.0]
